...
sub rudiServe { 
    ...
    # only is-single-suite installations support containerized apps servers
    my $suiteConfigFile = "$ENV{RUDI_DIR}/../_config.yml";
    $ENV{SUITE_MODE} = $suiteMode;
    $ENV{SUITE_NAME} = "";
    if(-f $suiteConfigFile){
        $suiteMode = $IS_SINGLE_SUITE;
        $suiteName = basename(dirname($ENV{RUDI_DIR}));
        $suiteDir = "$ENV{RUDI_DIR}/suites/definitive/$suiteName"; # only definitive repos have semantic version tags
        $ENV{SUITE_MODE} = $suiteMode;
    ...
    # determine if and how the installation supports Singularity
    ...
    if($runtime eq 'container' or $runtime eq 'singularity'){
        $isSingleSuite or 
            throwError("only is-single-suite installations support containerized apps servers", $command);
        $singularityLoad or 
            throwError("--runtime '$runtime' requires Singularity on system or via config/singularity.yml >> load-command", $command);
        $suiteSupportsContainers or 
            throwError("--runtime '$runtime' requires container support from the tool suite", $command);
    } 

    # dispatch the launch request to the proper handler
    if($isSingleSuite and $singularityLoad and $suiteSupportsContainers){
...
sub launchServerDirect {
    ...
    # TODO: this will run Dioxus app, not R
    ...
    my $libsPath = "$ENV{RUDI_DIR}/library";
    my $R_LIBRARY = "$libsPath/R-$R_VERSION"; 
    -d $R_LIBRARY or throwError(
        "FATAL: R library directory for R version $R_VERSION not found:\n". 
        "    $R_LIBRARY\n".
        "Have you installed the CLI or tool suite for this R version?\n",
        'server');
    ...
    exec "$load_libGit2; $rLoadCommand; Rscript -e '.libPaths(\"$R_LIBRARY\"); mdi::$serverCmd(rudiDir = \"$ENV{RUDI_DIR}\", port = $port $dataDir)'";
}
...
sub addBindMounts {
    ...
    my $userConfig = "$ENV{RUDI_DIR}/config/apps.yml"; 
    my %seen = map { $_ => 1 } (@$bound, @$toBind);
    foreach my $ymlFile ($userConfig){
        -f $ymlFile or next;
        my $yamls = loadYamlFromString( slurpFile($ymlFile) );
