//! This is a template for constructing a workflow tool using RuDI framework components.
//! 
...
use sub_module::MySubModule; // import sub-modules as needed

// Tool structure, for passing data workers to record processing functions.
struct Tool {
    my_worker: MySubModule,
}
...
pub fn stream() -> Result<(), Box<dyn Error>> {
    ...
    let mut tool = Tool {
        my_worker: MySubModule::new(&mut w),
    ...
    rs // choose whatever record_streamer and options match your input data format
        .group_by_in_place_serial(|alns: &mut Vec<MyRecord>| record_parser(
            alns, 
            &mut w,
            &mut tool,
        ), &["grouping_field"]);
